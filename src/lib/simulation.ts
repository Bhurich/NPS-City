...
function createBuilding(type: BuildingType): Building {
  // For NPS City, show placed buildings immediately so new players get clear feedback.
  const constructionProgress = 100;
...
export function createInitialGameState(size: number = DEFAULT_GRID_SIZE, cityName: string = 'New City'): GameState {
  ...
  return {
    ...
    gameVersion: 0,
    cities: [defaultCity],
    npsChallenge: createInitialNpsChallenge(),
  };
}
...
function evolveBuilding(grid: Tile[][], x: number, y: number, services: ServiceCoverage, demand?: { residential: number; commercial: number; industrial: number }): Building {
  ...
  const isStarterAnchor = isStarterBuilding(anchorX, anchorY, anchorBuilding.type);
  const efficiency = isStarterAnchor
    ? 1
    : (anchorBuilding.powered ? 0.5 : 0) + (anchorBuilding.watered ? 0.5 : 0);
...
function calculateStats(grid: Tile[][], size: number, budget: Budget, taxRate: number, effectiveTaxRate: number, services: ServiceCoverage, hour: number = 12): Stats {
  ...
  // Calculate income and expenses. NPS City is tuned for short employee campaigns,
...
export function simulateTick(state: GameState): GameState {
  ...
  const totalTicks = ((state.year - 2026) * 12 * 30 * 30) + ((state.month - 1) * 30 * 30) + ((state.day - 1) * 30) + newTick;
  ...
  if (newTick >= 30) {
    newTick = 0;
    newDay++;
    // Daily campaign payout: keeps the 14-day challenge moving quickly.
    newStats.money += Math.floor((newStats.income - newStats.expenses) / 10);
  }
  ...
  const npsChallenge = getNpsChallenge(state);
  const campaignDay = npsChallenge.mode === 'freebuild' ? 14 : getCampaignDay(npsChallenge);
  const completedMissionIds = new Set(npsChallenge.completedMissionIds);
  ...
  const challengeMetrics = getNpsMetrics(challengeState);

  for (const mission of NPS_MISSIONS) {
    if (mission.day > campaignDay || completedMissionIds.has(mission.id)) continue;
    const progress = getMissionProgress(mission, challengeMetrics, challengeState);
    if (!progress.complete) continue;

    completedMissionIds.add(mission.id);
    newStats.money += mission.reward;
    newNotifications.unshift({
      id: `nps-mission-${mission.id}-${Date.now()}`,
      title: `ภารกิจสำเร็จ: ${mission.title}`,
      description: `รับเงินรางวัล ${mission.reward.toLocaleString()} NPS Coin`,
      icon: 'trophy',
      timestamp: Date.now(),
    });
  }

  const updatedNpsChallenge = {
    ...npsChallenge,
  ...
  return {
    ...
    history,
    npsChallenge: updatedNpsChallenge,
  };
}
...
const BUILDING_SIZES: Partial<Record<BuildingType, { width: number; height: number }>> = {
  ...
  biomass_plantation: { width: 1, height: 1 },
  harvested_plantation: { width: 1, height: 1 },
  ...
  for (let dy = 0; dy < size.height; dy++) {
    for (let dx = 0; dx < size.width; dx++) {
      ...
      if (dx === 0 && dy === 0) {
        ...
        cell.building.age = 0;
        cell.building.population = Math.floor(stats.maxPop * Math.max(1, level) * 0.8);
        cell.building.jobs = Math.floor(stats.maxJobs * Math.max(1, level) * 0.8);
        if (services) {
