...
const TIP_DEFINITIONS: TipDefinition[] = [
  {
    id: 'get_started',
    message: msg('เริ่มจากวางถนนก่อน แล้ววางโซนที่อยู่อาศัย พาณิชย์ และอุตสาหกรรมไว้ข้างถนน จากนั้นกดเล่นเวลา ตึกและคนจะค่อยๆ เกิดขึ้นเอง'),
  ...
  {
    id: 'needs_utilities',
    message: msg('ถ้าวางโซนแล้วตึกยังไม่ขึ้น ให้เช็กว่ามีถนนเชื่อมอยู่หรือไม่ แล้วค่อยเพิ่มโรงไฟฟ้าและหอเก็บน้ำเพื่อให้เมืองโตต่อ'),
    priority: 1,
    check: (state: GameState) => {
...
export function useTipSystem(state: GameState): UseTipSystemReturn {
  ...
  const [shownTips, setShownTips] = useState<Set<TipId>>(() => {
