...
import { createInitialNpsChallenge } from '@/lib/npsChallenge';
import {
  Budget,
  BuildingType,
  GameState,
...
function migrateCampaignYear<T extends { year?: number }>(state: T): T {
  if ((state.year ?? 0) < 2026) {
    state.year = 2026;
  }
  return state;
}
  ...
  try {
    ...
    if (saved) {
      ...
      if (parsed?.state && parsed.state.grid && parsed.state.gridSize && parsed.state.stats) {
        migrateCampaignYear(parsed.state);
